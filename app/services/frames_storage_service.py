...
class FramesStorageService, redis_service):
    ...
    def process_video_to_frames(self, script_content, script_id, video_id):
        ...
        output_pattern = "frames_{}_%04d".format(unique_id)  # Se generarán archivos, etc.
        ...
        try= subprocess.run([
                ...
                '-w', '/scripts',
                'localhost,  # Imagen Docker que contiene Python y FFmpeg
            ...
            if frame_files= []
                for frame in frame_files, por ejemplo, 'image')
                    file_id = self.storage_service.save_file_to_mysql(frame, 'image')
                    file_ids.append(file_id)
                    os.remove(frame)  # Borrar la imagen después de subirla
                self.redis_service.push_result(script_id, "file_ids))
            else, "No se encontraron capturas. {}".format(result.stdout))
